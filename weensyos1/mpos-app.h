 ...
 *
 *   Create a new thread starting with an empty stack
 *   new thread should start by executing the start_function function: 
 *   that is, that function's address becomes the new thread's instruction pointer.
 *   sys_newthread() will only return thread_id successfully *once* for a given thread
 *   Returns -1 if the system call failed.
 *
 *****************************************************************************/
